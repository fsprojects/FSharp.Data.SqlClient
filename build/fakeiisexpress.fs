...
type IISExpressOptions = { ToolPath: string }
...
let createConfigFile (name, siteId: int, templateFileName, path, hostName, port: int) =
    ...
    let uniqueConfigFile =
        Path.Combine(Path.GetTempPath(), "iisexpress-" + Guid.NewGuid().ToString() + ".config")

    use template = File.OpenRead(templateFileName)
    let xml = XDocument.Load(template)

    let sitesElement =
        xml.Root.Element(xname "system.applicationHost").Element(xname "sites")

    let appElement =
        XElement(
            xname "site",
            XAttribute(xname "name", name),
            XAttribute(xname "id", siteId.ToString()),
            ...
            XElement(
                ...
                XElement(
                    xname "virtualDirectory",
                    XAttribute(xname "path", "/"),
                    XAttribute(xname "physicalPath", DirectoryInfo(path).FullName)
                )
            ),
...
[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
let OpenWebsiteInBrowser hostName port =
    sprintf "http://%s:%d/" hostName port |> OpenUrlInBrowser

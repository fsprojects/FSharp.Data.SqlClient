...
module internal Debug =
    ...
    let generate (resolutionFolder: string) (runtimeAssembly: string) typeProviderForNamespacesConstructor args =
        ...
        let (?<-) cfg prop value =
            cfg.GetType().GetProperty(prop).GetSetMethod(nonPublic = true).Invoke(cfg, [| box value |])
            |> ignore
        ...
        let typeProviderForNamespaces =
            typeProviderForNamespacesConstructor cfg :> TypeProviderForNamespaces
        ...
        | args ->
            let typeName =
                if providedTypeDefinition.IsErased then
                    ...
                    + (args
                       |> Seq.map (fun s -> ",\"" + (if s = null then "" else s.ToString()) + "\"")
                       |> Seq.reduce (+))
                else
    ...
    let prettyPrint signatureOnly ignoreOutput maxDepth maxWidth (t: ProvidedTypeDefinition) =
        ...
        let rec toString useFullName (t: Type) =
                ...
                | t when t.IsGenericType ->
                        ...
                        if useFullName then
                            ...
                            |> Seq.map (
                                if hasUnitOfMeasure then
                                    ...
                                    toString useFullName
                            )
                        else
                            t.GetGenericArguments() |> Seq.map (fun _ -> "_")
                    ...
                    if FSharpType.IsTuple t then
                        ...
                        let name, reverse =
                            ...
                            | t when t.GetGenericTypeDefinition().Name = typeof<int seq>.GetGenericTypeDefinition().Name ->
                                "seq", true
                            | t when
                                t.GetGenericTypeDefinition().Name = typeof<int list>.GetGenericTypeDefinition().Name
                                ->
                                "list", true
                            | t when
                                t.GetGenericTypeDefinition().Name = typeof<int option>.GetGenericTypeDefinition().Name
                                ->
                                "option", true
                            | t when t.GetGenericTypeDefinition().Name = typeof<int ref>.GetGenericTypeDefinition().Name ->
                                "ref", true
            ...
            let rec warnIfWrongAssembly (t: Type) =
                ...
                | t when t.IsGenericType ->
                    defaultArg
                        (t.GetGenericArguments()
                         |> Seq.map warnIfWrongAssembly
                         |> Seq.tryFind (fun s -> s <> ""))
                        ""
                | t when t.IsArray -> warnIfWrongAssembly <| t.GetElementType()
                | t ->
                    if not t.IsGenericParameter && t.Assembly = Assembly.GetExecutingAssembly() then
                        " [DESIGNTIME]"
                    else
                        ""
            ...
            if ignoreOutput then
                ...
                (toString useFullName t.DeclaringType)
                + "+"
                + (innerToString t)
                + (warnIfWrongAssembly t)
        ...
        let printExpr expr =
            ...
            let getCurrentIndent () =
                ...
                if lastEnterPos = -1 then
                    sb.Length + 4
                else
                    sb.Length - lastEnterPos - 1
            ...
            let inline hasAttr attrName m = not (Seq.isEmpty (getAttrs attrName m))
            ...
            and printExpr fromPipe needsParens =
                function
                | Call(instance, mi, args) ->
                    if
                        mi.Name = "GetArray"
                        && mi.DeclaringType.FullName = "Microsoft.FSharp.Core.LanguagePrimitives+IntrinsicFunctions"
                    then
                        ...
                        && mi.DeclaringType.GetGenericTypeDefinition().Name = typeof<int option>
                ...
                | Let(var, value, body) ->
                    ...
                    let usePattern =
                        sprintf
                            "IfThenElse(TypeTest(IDisposable,Coerce(%s,Object)),Call(Some(Call(None,UnboxGeneric,[Coerce(%s,Object)])),Dispose,[]),Value(<null>))"
                            var.Name
                            var.Name

                    let body =
                        match body with
                        | TryFinally(tryExpr, finallyExpr) when
                            finallyExpr.ToString().Replace("\n", null).Replace(" ", null) = usePattern
                            ->
                ...
                | Value(value, typ) when typ = typeof<string> && (value :?> string).Contains("\\") ->
                    bprintf sb "@%A" value
                | Value(value, _) -> bprintf sb "%A" value
                | Var(var) -> print var.Name
                | NewObject(ci, args) ->
                    ...
                    if
                        not (Seq.isEmpty compilationMappings)
                        && (getSourceConstructFlags (Seq.head compilationMappings)) = int
                ...
                | TupleGet(expr, index) ->
                    ...
                    let rec getTupleLength (typ: Type) =
                        ...
                        if length = 0 then // happens in the Apiary provider
                            ...
                            if lastItem.Name.StartsWith "Tuple`" then
                                length + getTupleLength lastItem - 1
                            else
                                length
                    ...
                    for i in 0 .. tupleLength - 1 do
                        if i = index then print varName else print "_"
        ...
        let printMember (memberInfo: MemberInfo) =
            ...
            let getMethodBody (m: ProvidedMethod) =
                seq {
                    if not m.IsStatic then
                        yield (ProvidedTypeDefinition.EraseType m.DeclaringType)

                    for param in m.GetParameters() do
                        yield (ProvidedTypeDefinition.EraseType param.ParameterType)
                }
            ...
            let getConstructorBody (c: ProvidedConstructor) =
                if c.IsImplicitConstructor then
                    Expr.Value(())
                else
                    seq {
                        for param in c.GetParameters() do
                            yield (ProvidedTypeDefinition.EraseType param.ParameterType)
                    }
                    |> Seq.map (fun typ -> Expr.Value(null, typ))
                    |> List.ofSeq
            ...
            let printObj x =
                if ignoreOutput then "" else sprintf "\n%O\n" x
            ...
            | :? ProvidedField as field when field.IsLiteral = true ->
                let value =
                    if signatureOnly then
                        ...
                        field.GetRawConstantValue() |> printObj

                if not ignoreOutput then
                    print
                    <| "val " + (getName field) + ": " + (toString true field.FieldType) + value

            | :? ProvidedProperty as prop ->
                if not ignoreOutput then
                    print
                    <| (if prop.IsStatic then "static " else "")
                       + "member "
                       + (getName prop)
                       + ": "
                       + (toString true prop.PropertyType)
                       + " with "
                       + (if prop.CanRead && prop.CanWrite then "get, set"
                          else if prop.CanRead then "get"
                          else "set")
            ...
            | :? ProvidedMethod as m ->
                if m.Attributes &&& MethodAttributes.SpecialName <> MethodAttributes.SpecialName then
                    if not ignoreOutput then
                        print
                        <| (if m.IsStatic then "static " else "")
                           + "member "
                           + (getName m)
                           + ": "
                           + (toSignature <| m.GetParameters())
                           + " -> "
        ...
        while pending.Count <> 0 && !currentDepth <= maxDepth do
            ...
            let pendingForThisDepth =
                pendingForThisDepth |> Seq.sortBy (fun m -> m.Name) |> Seq.truncate maxWidth
            ...
            for t in pendingForThisDepth do
                ...
                println ()
                t.GetMembers() |> Seq.sortBy (fun m -> m.Name) |> Seq.iter printMember
                println ()

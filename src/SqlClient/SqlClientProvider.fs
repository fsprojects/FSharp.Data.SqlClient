            ...
            |> List.map (fun (tableName, baseTableName, baseSchemaName, description) -> 
                    ...
                    for c in columns do
                        let property = 
                            ...
                            then
                                ...
                                let setterCode =
                                    if not c.ReadOnly then Some (QuotationsFactory.GetBody("SetNullableValueInDataRow", dataType, name)) else None
                                let property = ProvidedProperty(name, propertType, getterCode = QuotationsFactory.GetBody("GetNullableValueFromDataRow", dataType, name), ?setterCode = setterCode)
                                
                                property
                            else
                                let propertyGetterCode =
                                    ...
                                    then
                                        ...
                                        fun (args: Expr list) -> <@@ (%%args.[0] : DataRow).[name] @@>
                                
                                let propertySetterCode =
                                    if not c.ReadOnly then Some(fun (args: Expr list) -> <@@ (%%args.[0] : DataRow).[name] <- %%Expr.Coerce(args.[1], typeof<obj>) @@>) else None
                                
                                let property = ProvidedProperty(name, dataType, getterCode = propertyGetterCode, ?setterCode = propertySetterCode)
                ...
                do
                    ...
                    do 
                        ...
                        let addRowMethodInvokeCode = fun (args: Expr list) -> 
                            ...
                            <@@
                                ...
                                table.Rows.Add row
                            @@>
                        let addRowMethod = ProvidedMethod("AddRow", parameters, typeof<Void>, addRowMethodInvokeCode)

module FSharp.Data.SqlClient.DesignTime
...
let internal addGeneratedMethod (sqlParameters: Parameter list) (executeArgs: ProvidedParameter list) allParametersOptional (cmdProvidedType: ProvidedTypeDefinition) erasedType providedOutputType name =
...
let internal getRecordType(columns: Column list) =
...
let internal getDataRowType (outputColumns: Column list) = 
...
let internal getOutputTypes (outputColumns: Column list) (resultType: ResultType) (rank: ResultRank) =    
    ...
    then
        let dataRowType = getDataRowType outputColumns
        ...
        let providedRowType, erasedToRowType, rowMapping = 
            ...
            then 
                let r = getRecordType outputColumns
...
let internal getOutputColumns (connection: SqlConnection) commandText (parameters: Parameter list) isStoredProcedure = 
    try
...
let internal tryGetOutputColumns connection commandText parameters isStoredProcedure = 
    try
        getOutputColumns connection commandText parameters isStoredProcedure |> Some
    with _ ->
        None

let internal extractParameters connection (commandText: string) =  [
...
let internal getExecuteArgs (cmdProvidedType: ProvidedTypeDefinition) (sqlParameters: Parameter list) allParametersOptional (udtts: ProvidedTypeDefinition list) = 
    [

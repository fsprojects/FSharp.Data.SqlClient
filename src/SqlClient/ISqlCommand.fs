...
type ``ISqlCommand Implementation``(cfg: DesignTimeConfig, connection: Connection, commandTimeout) =
    ...
    let getReaderBehavior () =
        seq {
            ...
            if cmd.Connection.State <> ConnectionState.Open && manageConnection then
                cmd.Connection.Open()
                yield CommandBehavior.CloseConnection

            if cfg.Rank = ResultRank.SingleRow then
    ...
    let execute, asyncExecute, executeSingle, asyncExecuteSingle =
        ...
        | ResultType.Tuples ->
            ...
            | rowMapping, itemTypeName ->
                ...
                executeHandle.Invoke(null, [| ResultRank.SingleRow; cfg.RowMapping |])
                |> unbox
                >> box,
                asyncExecuteHandle.Invoke(null, [| ResultRank.SingleRow; cfg.RowMapping |])
                |> unbox
                >> box
    ...
    interface ISqlCommand with
        ...
        member this.Execute parameters =
            execute (cmd, getReaderBehavior, parameters, cfg.ExpectedDataReaderColumns)

        member this.AsyncExecute parameters =
            asyncExecute (cmd, getReaderBehavior, parameters, cfg.ExpectedDataReaderColumns)

        member this.ExecuteSingle parameters =
            executeSingle (cmd, getReaderBehavior, parameters, cfg.ExpectedDataReaderColumns)

        member this.AsyncExecuteSingle parameters =
            asyncExecuteSingle (cmd, getReaderBehavior, parameters, cfg.ExpectedDataReaderColumns)
        ...
        member this.ToTraceString parameters =
            ...
            let parameterDefinition (p: SqlParameter) =
                ...
                elif p.Size <> 0 && p.SqlDbType <> SqlDbType.Xml && p.SqlDbType <> SqlDbType.TinyInt then
            ...
            let getSqlDbType =
                let lookup =
                    Map.ofSeq
                    <| Seq.zip
                        (parameters |> Seq.map (fun (name, value) -> name))
                        ...
                        |> Seq.map (fun (name, value) ->
                            ...
                            | nonNullValue ->
                                let printedValue =
                                    ...
                                    | :? System.DateTime as d when getSqlDbType name = SqlDbType.DateTime2 ->
                                        d.ToString("O")
                                    // print dates with low precision (SQL datetime) in legacy format
                                    | :? System.DateTime as d when getSqlDbType name <> SqlDbType.DateTime2 ->
                                        d.ToString("yyyy-MM-ddTHH:mm:ss.fff")
                    ...
                    | SqlDbType.Structured ->
                        ...
                        let records =
                            typeof<Linq.Enumerable>
                                .GetMethod("Cast")
                                .MakeGenericMethod(sqlDataRecordType)
                                .Invoke(null, [| value |])
    ...
    static member internal VerifyResultsetColumns(cursor: SqlDataReader, expected) =
        if FsharpDataSqlClientConfiguration.Current.ResultsetRuntimeVerification then
            if cursor.FieldCount < Array.length expected then
                let message =
                    sprintf
                        "Expected at least %i columns in result set but received only %i."
                        expected.Length
                        cursor.FieldCount
            ...
            for i = 0 to expected.Length - 1 do
                let expectedName, expectedType =
                    fst expected.[i], Type.GetType(snd expected.[i], throwOnError = true)

                let actualName, actualType = cursor.GetName(i), cursor.GetFieldType(i)

                if actualName <> expectedName || actualType <> expectedType then
                    let message =
                        sprintf
                            """Expected column [%s] of type "%A" at position %i (0-based indexing) but received column [%s] of type "%A"."""
                            expectedName
                            expectedType
                            i
                            actualName
    ...
    static member internal ExecuteDataTable(cmd, getReaderBehavior, parameters, expectedDataReaderColumns) =
        use cursor =
            ``ISqlCommand Implementation``.ExecuteReader(cmd, getReaderBehavior, parameters, expectedDataReaderColumns)
    ...
    static member internal AsyncExecuteDataTable(cmd, getReaderBehavior, parameters, expectedDataReaderColumns) =
        async {
            use! reader =
                ``ISqlCommand Implementation``.AsyncExecuteReader(
                    cmd,
                    getReaderBehavior,
                    parameters,
                    expectedDataReaderColumns
                )
    ...
    static member internal ExecuteSeq<'TItem>(rank, rowMapper) =
        fun (cmd: SqlCommand, getReaderBehavior, parameters, expectedDataReaderColumns) ->
            let hasOutputParameters =
                cmd.Parameters
                |> Seq.cast<SqlParameter>
                |> Seq.exists (fun x -> x.Direction.HasFlag(ParameterDirection.Output))
    ...
    static member internal AsyncExecuteSeq<'TItem>(rank, rowMapper) =
        fun (cmd, getReaderBehavior, parameters, expectedDataReaderColumns) ->
            let xs =
                async {
                    let! reader =
                        ``ISqlCommand Implementation``.AsyncExecuteReader(

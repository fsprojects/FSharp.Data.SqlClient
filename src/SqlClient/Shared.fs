...
type Column =
    {
        ...
        [<DataMember>]
        Identity: bool
        /// Identity (autoincremented) column
        [<DataMember>]
    ...
    member this.ErasedToType =
        if this.Nullable then
            typedefof<_ option>.MakeGenericType this.TypeInfo.ClrType
        else
            this.TypeInfo.ClrType
#if DESIGNTIME_CODE_ONLY
    member this.GetProvidedType
        (unitsOfMeasurePerSchema: IDictionary<string, ProviderImplementation.ProvidedTypes.ProvidedTypeDefinition list>)
        =
        let typeConsideringUOM: Type =
            if this.TypeInfo.IsUnitOfMeasure then
                let uomType =
                    unitsOfMeasurePerSchema.[this.TypeInfo.Schema]
                    |> List.find (fun x -> x.Name = this.TypeInfo.UnitOfMeasureName)

                ProviderImplementation.ProvidedTypes.ProvidedMeasureBuilder.AnnotateType(
                    this.TypeInfo.ClrType,
                    [ uomType ]
                )
                ...
                let matchingUddtColumn =
                    this.TypeInfo.TableTypeColumns |> Array.find (fun t -> t.Name = this.Name)
...
and [<DataContract; CLIMutable>] TypeInfo =
    {
        ...
        [<DataMember>]
        Schema: string
        /// Schema owning this type (sys for system types)
        [<DataMember>]
        SqlEngineTypeId: int
        /// system_type_id
        [<DataMember>]
        ...
        [<DataMember>]
        SqlDbType: SqlDbType
        /// ADO.NET <see cref="SqlDbType"/>
        [<DataMember>]
        IsFixedLength: bool
        /// Is fixed length
        [<DataMember>]
        ClrTypeFullName: string
        /// Fullname of type, for use with runtime reflection
        [<DataMember>]
        UdttName: string
        /// Name of the user-defined-table-type
        [<DataMember>]
        TableTypeColumns: Column array /// Columns of the user-defined-table-type
    }

    member this.ClrType: Type =
        if isNull this.ClrTypeFullName then
            null
        else
            Type.GetType(this.ClrTypeFullName, throwOnError = true)
    ...
    member this.IsUnitOfMeasure =
        this.TypeName.StartsWith("<") && this.TypeName.EndsWith(">")
...
type Parameter =
    {
        ...
        /// ADO.NET <see cref="ParameterDirection"/>
        [<DataMember>]
        MaxLength: int
        /// Max Length for variable length types
        [<DataMember>]
    ...
    member this.Size =
        ...
        | SqlDbType.NChar
        | SqlDbType.NText
        | SqlDbType.NVarChar -> this.MaxLength / 2
        | _ -> this.MaxLength
#if DESIGNTIME_CODE_ONLY
    member this.GetProvidedType
        (unitsOfMeasurePerSchema: IDictionary<string, ProviderImplementation.ProvidedTypes.ProvidedTypeDefinition list>)
...
type TempTableLoader(fieldCount, items: obj seq) =
    ...
    interface IDataReader with
        ...
        member __.Item
            with get (_: int): obj = invalidOp "NotImplementedException"

        member __.Item
            with get (_: string): obj = invalidOp "NotImplementedException"

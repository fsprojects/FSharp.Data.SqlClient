...
"""
    ...
    member internal this.CreateRootType
        (typeName, query, connectionStringOrName, provider, configFile, kind: SqlEnumKind)
        =
        let tempAssembly = ProvidedAssembly()
        ...
        if not reader.HasRows then
            failwith "Resultset is empty. At least one row expected."
        ...
        let valueType, getValue =
            ...
            let getValueType (row: DataRow) =
                ...
                if not (t.IsValueType || Type.(=) (t, typeof<string>)) then
                    failwithf
                        "Invalid type %s of column %O for value part. Only .NET value types and strings supported as value."
                        t.FullName
                        row.["ColumnName"]
            ...
            | 1 -> //natural keys case. Thanks Ami for introducing me to this idea.
                ...
                let getValue (values: obj[]) =
                    values.[0], Expr.Value(values.[0], valueType)
            ...
            | 2 ->
                ...
                let getValue (values: obj[]) =
                    values.[1], Expr.Value(values.[1], valueType)

                valueType, getValue
            | _ ->
                let tupleItemTypes =
                    schema.Rows |> Seq.cast<DataRow> |> Seq.skip 1 |> Seq.map getValueType
        ...
        |> Seq.iter (fun (key, xs) ->
            if Seq.length xs > 1 then
                failwithf "Non-unique label %s." key)

        match kind with
        | SqlEnumKind.CLI ->

            if not (allowedTypesForEnum.Contains(valueType)) then
                failwithf
                    "Enumerated types can only have one of the following underlying types: %A."
                    [| for t in allowedTypesForEnum -> t.Name |]
        ...
        | _ ->
            ...
            let itemsExpr =
                Expr.NewArray(
                    itemType,
                    (names, values)
                    ||> List.map2 (fun name value -> Expr.NewTuple([ Expr.Value name; snd value ]))
                )
    ...
    static member internal GetTryParseImpl<'Value> items =
        fun (args: _ list) ->
            <@@
                ...
                %%items
                |> Array.tryPick (fun (name: string, value: 'Value) ->
                    if comparer.Equals(name, %%args.[0]) then
                        Some value
                    else
                        None)
            @@>
    ...
    static member internal GetParseImpl<'Value>(items, typeName) =
        fun (args: _ list) ->
            ...
            <@@
                ...
                | None ->
                    let errMsg =
                        sprintf @"Cannot convert value ""%s"" to type ""%s""" %%args.[0] typeName
    ...
    static member internal GetTryFindName<'Value> items =
        fun (args: _ list) ->
            <@@
                %%items
                |> Array.tryPick (fun (name: string, value: 'Value) ->
                    if Object.Equals(value, (%%args.[0]: 'Value)) then
                        Some name
                    else
                        None)
            @@>

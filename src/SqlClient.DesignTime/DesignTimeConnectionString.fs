...
type internal DesignTimeConnectionString =
    ...
    static member Parse(s: string, resolutionFolder, fileName) =
        ...
        | [| prefix; tail |] when prefix.Trim().ToLower() = "name" ->
            ...
            let value, provider =
                DesignTimeConnectionString.ReadFromConfig(name, resolutionFolder, fileName)
    ...
    static member ReadFromConfig(name, resolutionFolder, fileName) =
        ...
        | _, Lazy null ->
            ...
            <| KeyNotFoundException(
                message = sprintf "Cannot find name %s in <connectionStrings> section of %s file." name configFilename
            )
        | _, Lazy x ->
            let providerName =
            ...
            <@@
                ...
                then
                    ...
                    if isNull section then
                        raise
                        <| KeyNotFoundException(
                            message = sprintf "Cannot find name %s in <connectionStrings> section of config file." name
                        )
                    else
                        section.ConnectionString
            @@>

    member this.IsDefinedByLiteral =
        match this with
        | Literal _ -> true
        | _ -> false

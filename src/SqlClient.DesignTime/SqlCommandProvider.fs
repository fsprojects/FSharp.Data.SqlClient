...
type SqlCommandProvider(config: TypeProviderConfig) as this =
    ...
    let providerType =
        ProvidedTypeDefinition(assembly, nameSpace, "SqlCommandProvider", Some typeof<obj>, hideObjectMethods = true)
...
#if DEBUG
                Debug.WriteLine(
                    "snag during clearing of data type cache: "
                    + whoIsClearingCache
                    + "\n"
                    + (string e)
                )
...
"""
    ...
    member internal this.CreateRootType
        ...
        if connectionStringOrName.Trim() = "" then
        ...
        let uomsPerSchema =
            [|
               ...
               for schema in schemas do
                   yield
                       schema,
                       SqlProgrammabilityProvider.UnitsOfMeasure(designTimeConnectionString.Value, schema, sqlDataTypes) |]
            |> dict
        ...
        let returnType =
            DesignTime.GetOutputTypes(outputColumns, resultType, rank, noOutputParameters, uomsPerSchema)
        ...
        do
            ...
            if not (List.isEmpty outputColumns) then
                cmdProvidedType.AddMember(
                    ProvidedProperty(
                        ...
                        typeof<Column array>,
                        isStatic = true,
                        getterCode = fun _ -> <@@ (serializedColumns |> Encoding.deserialize): Column array @@>
                    )
                )
            ...
            if not (List.isEmpty parameters) then
                cmdProvidedType.AddMember(
                    ProvidedProperty(
                        "Parameters",
                        typeof<Parameter array>,
                        isStatic = true,
                        getterCode = fun _ -> <@@ (serializedParameters |> Encoding.deserialize): Parameter array @@>
                    )
                )
        ...
        do //AsyncExecute, Execute, and ToTraceString
            ...
            let executeArgs =
                DesignTime.GetExecuteArgs(
                    ...
                    parameters,
                    udttsPerSchema = null,
                    unitsOfMeasurePerSchema = uomsPerSchema
                )

...
type SqlProgrammabilityProvider(config: TypeProviderConfig) as this =
    ...
    do this.Disposing.Add <| fun _ -> sqlDataTypesCache.Clear whoIsClearing
...
"""
    ...
    override this.ResolveAssembly args =
        ...
        |> Array.tryFind (fun x ->
            AssemblyName.ReferenceMatchesDefinition(AssemblyName.GetAssemblyName x, AssemblyName args.Name))
        |> Option.map Assembly.LoadFrom
        |> defaultArg
        <| base.ResolveAssembly args

    member internal this.CreateRootType
        (typeName, connectionStringOrName, configFile, dataDirectory, useReturnValue, resultType)
        =
        if String.IsNullOrWhiteSpace connectionStringOrName then
            invalidArg "ConnectionStringOrName" "Value is empty!"

        let designTimeConnectionString =
            DesignTimeConnectionString.Parse(connectionStringOrName, config.ResolutionFolder, configFile)
        ...
        let databaseRootType =
            ProvidedTypeDefinition(assembly, nameSpace, typeName, baseType = Some typeof<obj>, hideObjectMethods = true)
        ...
        let tagProvidedType (t: ProvidedTypeDefinition) =
            t.AddMember(
                ProvidedProperty(
                    ...
                    typeof<string>,
                    isStatic = true,
                    getterCode = fun _ -> <@@ connectionStringOrName @@>
                )
            )
        ...
        let schemas =
            conn.GetUserSchemas()
            |> List.map (fun schema ->
                ProvidedTypeDefinition(schema, baseType = Some typeof<obj>, hideObjectMethods = true))
        ...
        for schemaType in schemas do
            ...
            do //Units of measure
                let xs =
                    SqlProgrammabilityProvider.UnitsOfMeasure(conn.ConnectionString, schemaType.Name, sqlDataTypes)
            ...
            do //User-defined table types
                ...
                udttsRoot.AddMembersDelayed
                <| fun () -> this.UDTTs(conn.ConnectionString, schemaType.Name, uomPerSchema)
    ...
    member internal __.UDTTs(connStr, schema, unitsOfMeasurePerSchema) =
        [ for t in sqlDataTypesCache.GetTypesForConnectionString connStr do
              if t.TableType && t.Schema = schema then
    ...
    static member internal UnitsOfMeasure(connStr, schema, sqlDataTypesForConnection: TypeInfo array) =
        [ for t in sqlDataTypesForConnection do
              if t.Schema = schema && t.IsUnitOfMeasure then
    ...
    member internal __.Routines
        (conn, schema, uddtsPerSchema, resultType, designTimeConnectionString, useReturnValue, unitsOfMeasurePerSchema)
        =
        [ use _ = conn.UseLocally()
          ...
          for routine in routines do
              let cmdProvidedType =
                  ProvidedTypeDefinition(
                      snd routine.TwoPartName,
                      Some typeof<``ISqlCommand Implementation``>,
                      hideObjectMethods = true
                  )

              do routine.Description |> Option.iter cmdProvidedType.AddXmlDoc

              cmdProvidedType.AddMembersDelayed
              <| fun () ->
                  [ use __ = conn.UseLocally()
                    ...
                    let rank =
                        if routine.Type = ScalarValuedFunction then
                    ...
                    let hasOutputParameters =
                        parameters
                        |> List.exists (fun x -> x.Direction.HasFlag(ParameterDirection.Output))

                    let returnType =
                        DesignTime.GetOutputTypes(
                            outputColumns,
                            resultType,
                            rank,
                            hasOutputParameters,
                    ...
                    do SharedLogic.alterReturnTypeAccordingToResultType returnType cmdProvidedType resultType

                    //ctors
                    let sqlParameters =
                        Expr.NewArray(typeof<SqlParameter>, parameters |> List.map QuotationsFactory.ToSqlParam)
                        ...
                        let expectedDataReaderColumns =
                            Expr.NewArray(
                                ...
                                [ for c in outputColumns ->
                                      Expr.NewTuple [ Expr.Value c.Name; Expr.Value c.TypeInfo.ClrTypeFullName ] ]
                            )
                    ...
                    let executeArgs =
                        DesignTime.GetExecuteArgs(cmdProvidedType, parameters, uddtsPerSchema, unitsOfMeasurePerSchema)

                    yield
                        upcast
                            DesignTime.AddGeneratedMethod(
                                parameters,
                                hasOutputParameters,
                                executeArgs,
                    ...
                    if not hasOutputParameters then
                        let asyncReturnType =
                            ProvidedTypeBuilder.MakeGenericType(typedefof<_ Async>, [ returnType.Single ])
                    ...
                    if returnType.PerRow.IsSome then
                        let providedReturnType =
                            ProvidedTypeBuilder.MakeGenericType(
                                typedefof<_ option>,
                                [ returnType.PerRow.Value.Provided ]
                            )

                        let providedAsyncReturnType =
                            ProvidedTypeBuilder.MakeGenericType(typedefof<_ Async>, [ providedReturnType ])
            ...
            |> List.map (fun (tableName, baseTableName, baseSchemaName, description) ->
                ...
                let descriptionSelector =
                    if isSqlAzure then
                        ...
                        "fn_listextendedproperty ('MS_Description', 'schema', @schema, 'table', @tableName, 'column', columns.name)"
                ...
                let columns =
                    cmd.ExecuteQuery(fun x ->
                        let c =
                            Column.Parse(
                                x,
                                (fun (system_type_id, user_type_id) ->
                                    findTypeInfoBySqlEngineTypeId (
                        ...
                        if c.DefaultConstraint <> "" && c.PartOfUniqueKey then
                ...
                do
                    for c in columns do
                            ...
                            if c.Nullable then
                                ...
                                let property =
                                    ProvidedProperty(
                                        ...
                                        propertType,
                                        getterCode =
                                            QuotationsFactory.GetBody("GetNullableValueFromDataRow", dataType, name),
                                        ?setterCode =
                                            if not c.ReadOnly then
                                                QuotationsFactory.GetBody("SetNullableValueInDataRow", dataType, name)
                                                |> Some
                                            else
                                                None
                                    )
                                ...
                                ProvidedProperty(
                                    ...
                                    dataType,
                                    getterCode =
                                        (if c.Identity && c.TypeInfo.ClrType <> typeof<int> then
                                             fun args ->
                                                 <@@
                                                     ...
                                                     let targetType =
                                                         Type.GetType(
                                                             %%Expr.Value(c.TypeInfo.ClrTypeFullName),
                                                             throwOnError = true
                                                         )
                        ...
                        if c.Description <> "" then
                            property.AddXmlDoc c.Description

                        dataRowType.AddMember property

                //type data table
                let dataTableType =
                    DesignTime.GetDataTableType(
                        tableName,
                        dataRowType,
                        columns,
                        SqlProgrammabilityTable(config.IsHostedExecution, connectionString, schema, tableName, columns)
                    )
                ...
                do
                    description
                    |> Option.iter (fun x -> dataTableType.AddXmlDoc(sprintf "<summary>%s</summary>" x))

                do
                    let parameters, updateableColumns =
                        [ for c in columns do
                              if not (c.Identity || c.ReadOnly) then
                                  let dataType = c.TypeInfo.ClrType

                                  let parameter =
                                      if c.NullableParameter then
                                          ProvidedParameter(
                                              c.Name,
                                              parameterType = typedefof<_ option>.MakeGenericType dataType,
                                              optionalValue = null
                                          )
                                      else
                                          ProvidedParameter(c.Name, dataType)
                            ...
                            [ for c in updateableColumns do
                                  if c.Description <> "" then
                                      let defaultConstrain =
                                          if c.HasDefaultConstraint then
                                              sprintf " Default constraint: %s." c.DefaultConstraint
                                          else
                                              ""

                                      yield
                                          sprintf "<param name='%s'>%s%s</param>" c.Name c.Description defaultConstrain ]
                    ...
                    let invokeCode =
                        fun (args: _ list) ->
                            ...
                            <@@
                                ...
                                let values: obj[] =
                                    %%Expr.NewArray(typeof<obj>,
                                                    [ for x in argsValuesConverted -> Expr.Coerce(x, typeof<obj>) ])

                                let namesOfUpdateableColumns: string[] =
                                    %%Expr.NewArray(typeof<string>,
                                                    [ for c in updateableColumns -> Expr.Value(c.Name) ])

                                let optionalParams: bool[] =
                                    %%Expr.NewArray(typeof<bool>,
                                                    [ for c in updateableColumns -> Expr.Value(c.NullableParameter) ])

                                Debug.Assert(
                                    values.Length = namesOfUpdateableColumns.Length,
                                    "values.Length = namesOfUpdateableColumns.Length"
                                )

                                Debug.Assert(
                                    values.Length = optionalParams.Length,
                                    "values.Length = optionalParams.Length"
                                )
                    ...
                    do
                        let newRowMethod =
                            ProvidedMethod("NewRow", parameters, dataRowType, invokeCode = invokeCode)

                        if methodXmlDoc <> "" then
                    ...
                    for c in columns do
                        ...
                        let columnProperty =
                            ProvidedProperty(
                                ...
                                getterCode =
                                    fun args ->
                                        <@@ (%%Expr.Coerce(args.[0], typeof<DataTable>): DataTable).Columns.[name] @@>
                            )

                        columnProperty.AddObsoleteAttribute(
                            sprintf "This property is deprecated, please use Columns.%s instead." name
                        )
    ...
    member internal this.AddCreateCommandMethod
        (
            conn,
            designTimeConnectionString,
            ...
            (fun methodName args ->
                ...
                let getMethodImpl =
                    lazy
                        ...
                        if
                            singleRow
                            && not (resultType = ResultType.Records || resultType = ResultType.Tuples)
                        then
                            invalidArg
                                "singleRow"
                                "SingleRow can be set only for ResultType.Records or ResultType.Tuples."

                        use __ = conn.UseLocally()

                        let parameters =
                            DesignTime.ExtractParameters(conn, sqlStatement, allParametersOptional)

                        let outputColumns =
                            if resultType <> ResultType.DataReader then
                                DesignTime.GetOutputColumns(conn, sqlStatement, parameters, isStoredProcedure = false)
                            else
                                []

                        let rank =
                            if singleRow then
                                ResultRank.SingleRow
                            else
                                ResultRank.Sequence
                        ...
                        let designTimeConfig =
                            ...
                            <@@
                                { SqlStatement = sqlStatement
                                  ...
                                  Parameters =
                                    %%Expr.NewArray(typeof<SqlParameter>,
                                                    parameters |> List.map QuotationsFactory.ToSqlParam)
                                  ResultType = resultType
                                  Rank = rank
                                  RowMapping = %%returnType.RowMapping
                                  ItemTypeName = %%returnType.RowTypeName

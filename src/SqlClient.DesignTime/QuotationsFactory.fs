...
type QuotationsFactory private () =
    ...
    static member internal GetBody(methodName, specialization, [<ParamArray>] bodyFactoryArgs: obj[]) =
        ...
        let bodyFactory =
            let mi =
                typeof<QuotationsFactory>.GetMethod(methodName, BindingFlags.NonPublic ||| BindingFlags.Static)
    ...
    static member internal ToSqlParam(p: Parameter) =
        ...
        <@@
            ...
            if not isFixedLength then
                x.Size <- %%Expr.Value p.Size

            x.Precision <- %%Expr.Value p.Precision
            x.Scale <- %%Expr.Value p.Scale

            if x.SqlDbType = SqlDbType.Structured then
                let typeName: string =
                    sprintf "%s.%s" (%%Expr.Value p.TypeInfo.Schema) (%%Expr.Value p.TypeInfo.UdttName)
    ...
    static member internal OptionToObj<'T> value =
        <@@
            match %%value with
            | Some(x: 'T) -> box x
            | None -> DbNull
    ...
    static member internal MapArrayObjItemToOption<'T>(arr, index) =
        <@
            ...
            values.[index] <-
                box
                <| if Convert.IsDBNull(values.[index]) then
                       None
                   else
                       Some(unbox<'T> values.[index])
        @>

    static member internal MapArrayNullableItems(outputColumns: Column list, mapper: string) =
        let columnTypes, isNullableColumn =
            outputColumns
            |> List.map (fun c -> c.GetTypeInfoConsideringUDDT().ClrTypeFullName, c.Nullable)
            |> List.unzip
    ...
    static member internal SetNullableValueInDataRow<'T>(exprArgs: Expr list, name: string) =
        <@
            (%%exprArgs.[0]: DataRow).[name] <-
                match (%%exprArgs.[1]: option<'T>) with
                | None -> DbNull

...
type SqlFileProvider(config: TypeProviderConfig) =
    ...
    override __.CreateRootType(assembly, nameSpace, typeName, args) =
        let path, resolutionFolder, encoding =
            string args.[0], string args.[1], string args.[2]

        if Path.GetExtension(path) <> ".sql" then
            failwith "Only files with .sql extension are supported"
        ...
        let fullPath =
            if Path.IsPathRooted(path) then
                ...
                let parent =
                    ...
                    elif Path.IsPathRooted(resolutionFolder) then
                        resolutionFolder
                    else
                        Path.Combine(config.ResolutionFolder, resolutionFolder)
            ...
            lazy
                ...
                let content =
                    if encoding = "" then
                        ...
                        File.ReadAllText(fullPath, encoding = Encoding.GetEncoding(encoding))

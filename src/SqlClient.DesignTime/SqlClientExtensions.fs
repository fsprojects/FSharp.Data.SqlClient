...
type internal TypeInfoPerConnectionStringCache() =
    ...
    member x.Clear(reason: string) =
        lock (fun () -> dataTypeMappings.Clear())
            ...
            if dataTypeMappings.ContainsKey connectionString then
                ...
                if existingTypes = types then
                    System.Diagnostics.Debug.WriteLine(
                        sprintf
                            "types existing for connection %s, old types and new types are identical"
                            connectionString
                    )
                else
                    System.Diagnostics.Debug.WriteLine(
                        sprintf
                            "types existing for connection %s, old types and new types are different"
                            connectionString
                    )
#endif
            ...
            | false, _ ->
                raise (
                    new InvalidOperationException(
                        sprintf "types for connection %s were not retrieved!" connectionString
                    )
                ))
...
let internal findTypeInfoByProviderType (connStr, sqlDbType) =
    ...
    sqlDataTypesCache.GetTypesForConnectionString connStr
    |> Array.find (fun x -> x.SqlDbType = sqlDbType)
...
let rec parseDefaultValue (definition: string) (expr: Microsoft.SqlServer.TransactSql.ScriptDom.ScalarExpression) =
    ...
    | :? Microsoft.SqlServer.TransactSql.ScriptDom.Literal as x ->
        ...
        | _ -> None
    | :? UnaryExpression as x when
        x.UnaryExpressionType
        <> Microsoft.SqlServer.TransactSql.ScriptDom.UnaryExpressionType.BitwiseNot
        ->
...
type internal Routine =
    ...
    member this.ToCommantText(parameters: Parameter list) =
        ...
        | TableValuedFunction ->
            ...
            |> String.concat ", "
            |> sprintf "SELECT * FROM %s(%s)" twoPartNameIdentifier
        | ScalarValuedFunction ->
            parameters
            |> List.map (fun p -> p.Name)
            |> String.concat ", "
            |> sprintf "SELECT %s(%s)" twoPartNameIdentifier
...
type SqlConnection with
    ...
    member internal this.CheckVersion() =
        ...
        if int majorVersion < 11 then
            failwithf
                "Minimal supported major version is 11 (SQL Server 2012 and higher or Azure SQL Database). Currently used: %s"
                this.ServerVersion
    ...
    member internal this.GetUserSchemas() =
        ...
        use cmd =
            this.CreateCommand(CommandText = "SELECT name FROM sys.schemas WHERE principal_id = 1")
    ...
    member internal this.GetParameters(routine: Routine, isSqlAzure, useReturnValue) =
        ...
        let descriptionSelector =
            if isSqlAzure then
                ...
                sprintf
                    "fn_listextendedproperty ('MS_Description', 'schema', OBJECT_SCHEMA_NAME(object_id), '%s', OBJECT_NAME(object_id), 'PARAMETER', p.name)"
                    routineType
        ...
        [ use cmd = this.CreateCommand(CommandText = query)
          ...
          while cursor.Read() do
              ...
              let direction =
                  if unbox cursor.["suggested_is_output"] then
              ...
              let typeInfo =
                  findTypeInfoBySqlEngineTypeId (this.ConnectionString, system_type_id, user_type_id)

              let defaultValue =
                  match paramDefaults.Result.TryGetValue(name) with
                  | true, value -> value
                  | false, _ -> None
          ...
          if routine.IsStoredProc && useReturnValue then
              yield
                  { Name = "@RETURN_VALUE"
    ...
    member internal this.GetTables(schema, isSqlAzure) =
        ...
        cmd.ExecuteQuery(fun x ->
            string x.["Name"], string x.["BaseTableName"], string x.["BaseObjectSchema"], x.TryGetValue("DESCRIPTION"))
        |> Seq.toList
    ...
    member internal this.GetFullQualityColumnInfo commandText =
        ...
        use cmd =
            this.CreateCommand(
                CommandText = "sys.sp_describe_first_result_set",
...
"""
                ...
                let getProvidedType name is_user_defined is_table_type system_type_id user_type_id =
                    ...
                    | false, _ when is_user_defined && not is_table_type ->
                        ...
                        | true, item -> Some item
                    | false, _ when is_table_type -> Some(SqlDbType.Structured, null, false)
                    | _ -> failwith ("Unexpected type: " + name)

                let getProvidedTypeForSqlTypeEntry (x: SqlTypeEntry) =
                    getProvidedType x.name x.is_user_defined x.is_table_type x.system_type_id x.user_type_id
                ...
                let typeInfos =
                    [|
                       ...
                       for { name = name
                             ...
                             is_user_defined = is_user_defined } in sqlEngineTypes.Values do

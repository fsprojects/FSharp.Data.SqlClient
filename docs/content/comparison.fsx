...
-----------------------------------------
 ...
 and [SqlEntityConnection][ef]/[EdmxFile][edmx].  But I would like to point out for reasons you might prefer SqlCommandProvider over those. 
 ...
 the same issue). So, what's a deal? The thing is that you can have perfectly valid F# code that compiles but still fails at run-time because
 unsupported F#-to-SQL translation semantics. Contrary, SqlCommandProvider is "What You See Is What You Get" approach. 
 Once F# code involving SqlCommandProvider passes compilation stage you guaranteed to have valid executable (both F# code and T-SQL). 
 ...
 Evolution and changes in T-SQL code generated by type provider can presents certain problems too. 

...
type GetEvenNumbers =
    SqlCommandProvider<"select * from (values (2), (4), (8), (24)) as T(value)", ConnectionStrings.AdventureWorksNamed>
...
let emptyResultset () =
    use cmd =
        new SqlCommandProvider<"SELECT 42 WHERE 0 > 1", ConnectionStrings.AdventureWorksNamed>()
...
let TinyIntConversion () =
    use cmd =
        new SqlCommandProvider<
            "SELECT CAST(10 AS TINYINT) AS Value",
...
let ConditionalQuery () =
    use cmd =
        new SqlCommandProvider<
            "IF @flag = 0 SELECT 1, 'monkey' ELSE SELECT 2, 'donkey'",
            ConnectionStrings.AdventureWorksNamed,
            SingleRow=true,
...
type DeleteBitCoin =
    SqlCommandProvider<"DELETE FROM Sales.Currency WHERE CurrencyCode = @Code", ConnectionStrings.AdventureWorksNamed>
...
type GetBitCoin =
    SqlCommandProvider<
        "SELECT CurrencyCode, Name FROM Sales.Currency WHERE CurrencyCode = @code",
        ConnectionStrings.AdventureWorksNamed
     >
...
let asyncCustomRecord () =
    ...
    Assert.Equal(1, cmd.AsyncExecute("USD") |> Async.RunSynchronously |> Seq.length)

[<Fact>]
let singleRowOption () =
    use noneSingleton =
        new SqlCommandProvider<"select 1 where 1 = 0", ConnectionStrings.AdventureWorksNamed, SingleRow=true>()

    Assert.IsNone <| noneSingleton.Execute()

    use someSingleton =
        new SqlCommandProvider<"select 1", ConnectionStrings.AdventureWorksNamed, SingleRow=true>()
...
let ToTraceString () =
    ...
    let expected =
        sprintf
            "exec sp_executesql N'SELECT CAST(@Date AS DATE), CAST(@Number AS INT)',N'@Date Date,@Number Int',@Date=N'%s',@Number=N'%d'"
            universalPrintedNow
            num

    let cmd =
        new SqlCommandProvider<
            "SELECT CAST(@Date AS DATE), CAST(@Number AS INT)",
            ConnectionStrings.AdventureWorksNamed,
            ResultType.Tuples
         >()

    Assert.Equal<string>(expected, actual = cmd.ToTraceString(now, num))
...
let ``ToTraceString for dates`` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@Date AS DATE)", ConnectionStrings.AdventureWorksNamed>()
...
let ``ToTraceString for times`` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@Time AS Time)", ConnectionStrings.AdventureWorksNamed>()
...
let ``ToTraceString for tinyint`` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@ti AS TINYINT)", ConnectionStrings.AdventureWorksNamed>()
...
let ``ToTraceString for xml`` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@x AS XML)", ConnectionStrings.AdventureWorksNamed>()
...
let ``Roundtrip ToTraceString for unicode`` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@x AS NVARCHAR(20))", ConnectionStrings.AdventureWorksNamed>()
...
let ``Roundtrip ToTraceString for decimals with maximum precision`` () =
    ...
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@x AS DECIMAL(29, 19))", ConnectionStrings.AdventureWorksNamed>()
...
let ``Roundtrip ToTraceString for date time `` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@x AS DATETIME)", ConnectionStrings.AdventureWorksNamed>()
...
let ``Roundtrip ToTraceString for datetime2 `` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@x AS DATETIME2)", ConnectionStrings.AdventureWorksNamed>()
...
let ``Roundtrip ToTraceString for date time offsets`` () =
    let cmd =
        new SqlCommandProvider<"SELECT CAST(@x AS DATETIMEOFFSET)", ConnectionStrings.AdventureWorksNamed>()
...
let ``ToTraceString for CRUD`` () =
    ...
    Assert.Equal<string>(
        expected =
            "exec sp_executesql N'SELECT CurrencyCode, Name FROM Sales.Currency WHERE CurrencyCode = @code',N'@code NChar(3)',@code=N'BTC'",
        actual = let cmd = new GetBitCoin() in cmd.ToTraceString(bitCoinCode)
    )

    Assert.Equal<string>(
        expected =
            "exec sp_executesql N'INSERT INTO Sales.Currency VALUES(@Code, @Name, GETDATE())',N'@Code NChar(3),@Name NVarChar(50)',@Code=N'BTC',@Name=N'Bitcoin'",
        actual = let cmd = new InsertBitCoin() in cmd.ToTraceString(bitCoinCode, bitCoinName)
    )

    Assert.Equal<string>(
        expected =
            "exec sp_executesql N'DELETE FROM Sales.Currency WHERE CurrencyCode = @Code',N'@Code NChar(3)',@Code=N'BTC'",
...
let ``ToTraceString double-quotes`` () =
    use cmd =
        new SqlCommandProvider<"SELECT OBJECT_ID('Sales.Currency')", ConnectionStrings.AdventureWorksNamed>()
...
let ``ToTraceString double-quotes in parameter`` () =
    ...
    Assert.Equal<string>(
        expected =
            "exec sp_executesql N'SELECT * FROM Sales.Currency WHERE CurrencyCode = @CurrencyCode',N'@CurrencyCode NChar(3)',@CurrencyCode=N'A''B'",
...
let CommandTimeout () =
    use cmd =
        new SqlCommandProvider<
            "WAITFOR DELAY '00:00:06'; SELECT 42",
            ConnectionStrings.AdventureWorksNamed,
            SingleRow=true
         >(
            commandTimeout = 60
...
let DynamicSql () =
    ...
    Assert.Equal(
        51,
        cmd.Execute(
            "SELECT CONCAT(FirstName, LastName) AS Name, rowguid AS UUID FROM Person.Person WHERE FirstName = @p1",
            "Alex"
        )
        |> Seq.toArray
        |> Array.length
    ...
    Assert.Equal(
        9,
        cmd.Execute(
            "SELECT CONCAT(FirstName, LastName) AS Name, rowguid AS UUID FROM Person.Person WHERE FirstName = @p1 AND EmailPromotion = 2",
            "Alex"
        )
        |> Seq.toArray
        |> Array.length
    ...
    Assert.Equal(
        1,
        cmd.Execute("SELECT Name, rowguid AS UUID FROM Production.Product WHERE Name = @p1", "Chainring Nut")
        |> Seq.toArray
        |> Array.length
    )
...
let ConcurrentReaders () =
    let cmd =
        new GetEvenNumbers(ConnectionStrings.AdventureWorksLiteralMultipleActiveResults)
...
let ResultsetRuntimeVerificationLessThanExpectedColumns () =
    ...
    try
        ...
        Assert.Equal<string>("Expected at least 3 columns in result set but received only 2.", err.Message)
    finally
        SqlClient.Configuration.Current <- { ResultsetRuntimeVerification = false }

    let err =
        Assert.Throws<IndexOutOfRangeException>(fun () -> cmd.Execute() |> Seq.toArray |> ignore)

    Assert.Equal<string>("Index was outside the bounds of the array.", err.Message)
...
let ResultsetRuntimeVerificationDiffColumnTypes () =
    ...
    let err =
        Assert.Throws<InvalidCastException>(fun () -> cmd.Execute() |> Seq.toArray |> ignore)
    ...
    Assert.Equal<string>(expectedErrorMsg, err.Message)

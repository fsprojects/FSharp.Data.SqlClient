...
let getSequenceWithParams () =
    ...
    let xs =
        [ for x in cmd.Execute(topN = 3L, regionName = "United States", salesMoreThan = 1000000M) ->
              x.FirstName, x.LastName, x.SalesYTD ]
...
let udttAndTuplesOutput () =
    let cmd =
        DB.CreateCommand<"exec Person.myProc @x", ResultType.Tuples, SingleRow=true>()
...
let optionalParams () =
    use cmd =
        DB.CreateCommand<
            "SELECT CAST(@x AS INT) + ISNULL(CAST(@y AS INT), 1)",
            SingleRow=true,
...
[<Literal>]
let evenNumbers =
    "select value, svalue = cast(value as char(1)) from (values (2), (4), (6), (8)) as T(value)"
...
module TraceTests =
    ...
    [<Literal>]
    let queryStart = "SELECT CAST(@Value AS "
    ...
    [<Literal>]
    let queryDATETIME = queryStart + DATETIME + queryEnd

    [<Literal>]
    let DATETIMEOFFSET = "DateTimeOffset"

    [<Literal>]
    let queryDATETIMEOFFSET = queryStart + DATETIMEOFFSET + queryEnd

    [<Literal>]
    let TIMESTAMP = "Time"

    [<Literal>]
    let queryTIMESTAMP = queryStart + TIMESTAMP + queryEnd

    [<Literal>]
    let INT = "Int"

    [<Literal>]
    let queryINT = queryStart + INT + queryEnd

    [<Literal>]
    let DECIMAL63 = "Decimal(6,3)"

    [<Literal>]
    let queryDECIMAL63 = queryStart + DECIMAL63 + queryEnd

    let inline testTraceString traceQuery (cmd: ^cmd) dbType (value: ^value) printedValue =
        let expected =
            sprintf "exec sp_executesql N'%s',N'@Value %s',@Value=N'%s'" traceQuery dbType printedValue
    ...
    let traceDate () =
        ...
        testTraceString
            queryDATETIME
            (DB.CreateCommand<queryDATETIME>())
            DATETIME
            now
    ...
    let traceDateTimeOffset () =
        ...
        testTraceString
            queryDATETIMEOFFSET
            (DB.CreateCommand<queryDATETIMEOFFSET>())
            DATETIMEOFFSET
            now
            (now.ToString("O"))
    ...
    let traceTimestamp () =
        ...
        testTraceString
            queryTIMESTAMP
            (DB.CreateCommand<queryTIMESTAMP>())
            TIMESTAMP
            timeOfDay
            (timeOfDay.ToString("c"))

    [<Fact>]
    let traceInt () =
        testTraceString queryINT (DB.CreateCommand<queryINT>()) INT 42 "42"

    [<Fact>]
    let traceDecimal () =
        testTraceString
            queryDECIMAL63
            (DB.CreateCommand<queryDECIMAL63>())
            DECIMAL63
            123.456m
            (123.456m.ToString(System.Globalization.CultureInfo.InvariantCulture))

    [<Fact>]
    let traceNull () =
        let expected =
            sprintf "exec sp_executesql N'SELECT CAST(@Value AS NVARCHAR(20))',N'@Value NVarChar(20)',@Value=NULL"

        Assert.Equal<string>(
            expected,
            actual =
                DB.CreateCommand<"SELECT CAST(@Value AS NVARCHAR(20))">().ToTraceString(Unchecked.defaultof<string>)
        )
...
let resultSetMapping () =
    let cmd =
        DB.CreateCommand<
            "SELECT * FROM (VALUES ('F#', 2005), ('Scala', 2003), ('foo bar',NULL))  AS T(lang, DOB)",
    ...
    let expected =
        [| "F#", Some 2005; "Scala", Some 2003; "foo bar", None |]

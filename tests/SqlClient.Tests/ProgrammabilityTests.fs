...
type AdventureWorksDataTables =
    SqlProgrammabilityProvider<ConnectionStrings.AdventureWorksLiteral, ResultType=ResultType.DataTable>
...
let TableValuedFunction () =
    ...
    let expected =
        GetContactInformation.Record(
            PersonID = 1,
            FirstName = Some "Ken",
            LastName = Some "Sánchez",
            JobTitle = Some "Chief Executive Officer",
            BusinessEntityType = Some "Employee"
        )
...
type AdventureWorksFromDesignOnly =
    SqlProgrammabilityProvider<ConnectionStrings.AdventureWorksDesignOnly, ConfigFile="appWithInclude.config">
...
let ``no bad error message when db is changed before table update `` () =
    ...
    use cmd =
        new SqlCommand("EXEC sp_rename 'Sales.SalesTerritory', 'SalesTerr';", conn, tran)
    ...
    Assert.Throws<SqlException>(fun _ -> table.Update(conn, tran) |> ignore)
    |> ignore
...
let localTransactionCtor () =
    ...
    do
        //let get
        use updatedJobTitle =
            new AdventureWorks.HumanResources.uspUpdateEmployeeHireInfo (transaction = tran)
...
let SpWithParamOfTvpWithNullableColumns () =
    ...
    let p =
        [ DboMyTableType(myId = 1); DboMyTableType(myId = 2, myName = Some "donkey") ]

    Assert.Equal<_[]>([| 1, None; 2, Some "donkey" |], [| for x in cmd.Execute(p) -> x.myId, x.myName |])
...
let ``honors result type parameter: datatable`` () =
    let command =
        new AdventureWorksDataTables.Sales.GetUKSalesOrders(ConnectionStrings.AdventureWorksLiteral)

    let gbp = 1.0M<AdventureWorksDataTables.Sales.``Units of Measure``.GBP>

    let table: AdventureWorksDataTables.Sales.GetUKSalesOrders.Table =
        command.Execute(gbp)
...
let StaticCreateMethod () =
    let input1 =
        [ AdventureWorks.Person.``User-Defined Table Types``.MyTableType(42, Some "BSL") ]
    ...
    (newResult, factoryResult)
    ||> Seq.iter2 (fun newR facR ->
    ...
    use newCmd =
        new AdventureWorks.Person.MyProc(ConnectionStrings.AdventureWorksLiteral, 58)

    let newResult = newCmd.Execute(input2)

    use factoryCmd =
        AdventureWorks.Person.MyProc.Create(ConnectionStrings.AdventureWorksLiteral, 58)

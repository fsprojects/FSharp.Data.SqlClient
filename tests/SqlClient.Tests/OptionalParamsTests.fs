...
let NullableStringInputParameter () =
    use cmd =
        new SqlCommandProvider<
            "select ISNULL(CAST(@P1 AS VARCHAR), '')",
            connection,
            SingleRow=true,
            AllParametersOptional=true
         >()

    Assert.Equal(expected = Some "", actual = cmd.Execute(None))

    Assert.Equal(expected = Some "boo", actual = cmd.Execute(Some "boo"))

[<Fact>]
let NullableStringInputMandatoryParameter () =
    use cmd =
        new SqlCommandProvider<"select ISNULL(CAST(@P1 AS VARCHAR), '')", connection, SingleRow=true>()

    Assert.Equal(expected = Some "", actual = cmd.Execute(null))

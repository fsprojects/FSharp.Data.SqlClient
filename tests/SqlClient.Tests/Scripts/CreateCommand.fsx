...
getTopSalespeople.Execute(topN = 3L, regionName = "United States", salesMoreThan = 1000000M)
|> printfn "%A"

let get42AndTime =
    DB.CreateCommand<"SELECT 42, GETDATE()", ResultType.Tuples, SingleRow=true>(
        SqlClient.Connection.Instance connection
    )

get42AndTime.AsyncExecute() |> Async.RunSynchronously |> printfn "%A"

let myPerson =
    DB.CreateCommand<"exec Person.myProc @x", ResultType.Tuples, SingleRow=true, TypeName="MyProc">()

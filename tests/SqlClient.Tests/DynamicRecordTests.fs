...
let recordWithNulls =
    DynamicRecord(dict [ "DBNull", box DBNull.Value; "null", null; "foo", box "bar" ])
...
let ``TryGetMember succeeds`` () =
    Assert.Equal(expected = (true, box DBNull.Value), actual = recordWithNulls.TryGetMember(Binder("DBNull")))

[<Fact>]
let ``TryGetMember fails`` () =
    Assert.Equal(expected = recordWithNulls.TryGetMember(Binder("foobar")), actual = (false, null))
...
let ``Not equal with different keys`` () =
    let condition =
        recordWithNulls = DynamicRecord(dict [ "DBNull", box DBNull.Value; "bar", null ])
...
let ``Not equal with different values`` () =
    let condition =
        recordWithNulls = DynamicRecord(dict [ "DBNull", box DBNull.Value; "foo", box "foo" ])
...
let ``GetHashCode is same when equal`` () =
    ...
    if
        recordWithNulls = clone //did if to make assertion more granular
    then
...
let dateRecordString =
    """{"Date":"2012-01-01T00:00:00","Offset":"2012-01-01T00:00:00+02:00"}"""

let recordWithNullsString = """{"DBNull":null,"null":null,"foo":"bar"}"""

[<Fact>]
let ``JSON serialize`` () =
    Assert.Equal<string>(expected = recordWithNullsString, actual = JsonConvert.SerializeObject recordWithNulls)

    Assert.Equal<string>(expected = dateRecordString, actual = JsonConvert.SerializeObject(dateRecord))

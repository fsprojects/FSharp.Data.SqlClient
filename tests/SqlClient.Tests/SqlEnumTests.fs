...
type EnumMapping =
    SqlEnumProvider<
        "SELECT * FROM (VALUES(('One'), 1), ('Two', 2)) AS T(Tag, Value)",
...
let parse () =
    ...
    Assert.Throws<ArgumentException>(fun () -> box (TinyIntMapping.Parse("blah-blah")))
    |> ignore

    Assert.Throws<ArgumentException>(fun () -> box (TinyIntMapping.Parse("one")))
    |> ignore
...
type SingleColumnSelect =
    SqlEnumProvider<"SELECT Name FROM Purchasing.ShipMethod", ConnectionStrings.AdventureWorksNamed>
    ...
    let all =
        use cmd =
            new SqlCommandProvider<
                "SELECT Name, Name FROM Purchasing.ShipMethod",
...
let PatternMatchingOn () =
    ...
    Assert.Equal<_ seq>(SingleColumnSelect.Items |> Seq.map fst, actual)

type MoreThan2Columns =
    SqlEnumProvider<
        @"
select * from (
values 
...
let MoreThan2ColumnReturnsCorrectTuples () =
    ...
    Assert.Equal<_[]>(
        expected = [| ("a", (1, "this is a")); ("b", (2, "this is b")); ("c", (3, "this is c")) |],
...
type ProductCategory =
    SqlEnumProvider<"SELECT Name FROM Production.ProductCategory", ConnectionStrings.AdventureWorksLiteral>
          ...
          for x in cmd.Execute(ProductCategory.Bikes) do
              yield
                  { Id = x.ProductID
                    ...
                    Size =
                      x.Size
                      |> Option.map (fun size -> size |> float |> LanguagePrimitives.FloatWithMeasure) } ]

    let bigBikes =
        allBikes
        |> List.choose (fun x ->
            if x.Size = Some 52.<ProductsUnitsOfMeasure.``CM ``> then
                Some x.Name
            else
                None)

...
let implicitAsync () =
    ...
    begin
        ...
        Assert.Equal(
            1,
            (new InsertBitCoin()).AsyncExecute(bitCoinCode, bitCoinName)
            |> Async.RunSynchronously
        )

        Assert.Equal(
            1,
            (new GetBitCoin()).AsyncExecute(bitCoinCode)
            |> Async.RunSynchronously
            |> Seq.length
        )
...
let implicitAsyncNETBefore451 () =
    ...
    begin
        ...
        Assert.Equal(
            1,
            (new InsertBitCoin(transaction = localTran)).AsyncExecute(bitCoinCode, bitCoinName)
            |> Async.RunSynchronously
        )

        Assert.Equal(
            1,
            (new GetBitCoin(transaction = localTran)).AsyncExecute(bitCoinCode)
            |> Async.RunSynchronously
            |> Seq.length
        )
...
type RaiseError =
    SqlCommandProvider<"SELECT 42; THROW 51000, 'Error raised.', 1 ", ConnectionStrings.AdventureWorksNamed>
...
let donNotOpenConnectionOnObject () =
    ...
    use cmd =
        new SqlCommandProvider<"SELECT 42", ConnectionStrings.AdventureWorksNamed>(conn)

    Assert.Throws<InvalidOperationException>(fun () -> cmd.Execute() |> Seq.toArray |> ignore)

type NonQuery =
    SqlCommandProvider<"DBCC CHECKIDENT ('HumanResources.Shift', RESEED, 4)", ConnectionStrings.AdventureWorksNamed>
...
let prematurelyOpenConnection () =
    let cmd =
        new SqlCommandProvider<"SELECT 42", ConnectionStrings.AdventureWorksNamed>()

    let _ = cmd.Execute()

    Assert.Equal(ConnectionState.Closed, (cmd :> ISqlCommand).Raw.Connection.State)

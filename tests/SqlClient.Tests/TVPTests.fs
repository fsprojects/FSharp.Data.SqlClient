...
type TableValuedTuple =
    SqlCommandProvider<
        "exec Person.myProc @x",
...
let NullableColumn () =
    let cmd = new TableValuedTuple()
    let p = [ MyTableType(myId = 1); MyTableType(myId = 2, myName = Some "donkey") ]
    Assert.Equal(Some(1, None), cmd.Execute p)

type TableValuedSingle =
    SqlCommandProvider<"exec SingleElementProc @x", ConnectionStringOrName=ConnectionStrings.AdventureWorksNamed>
...
type TableValuedSprocTuple =
    SqlCommandProvider<
        "exec Person.myProc @x",
        ConnectionStringOrName=ConnectionStrings.AdventureWorksNamed,
        SingleRow=true,
        ResultType=ResultType.Tuples
     >
...
type TableValuedTupleWithOptionalParams =
    SqlCommandProvider<"exec Person.myProc @x", ConnectionStrings.AdventureWorksNamed, AllParametersOptional=true>
...
let ReuseTVPTypeForDynamicADONET () =
    ...
    p.Value <-
        [ MyTableType(myId = 1, myName = Some "monkey")
          MyTableType(myId = 2, myName = Some "donkey") ]
        |> Seq.cast<Microsoft.Data.SqlClient.Server.SqlDataRecord>
...
type MappedTVP =
    ...
    ",
        ...
        TableVarMapping="@input=dbo.MyTableType"
     >
...
type MappedTVPFixed =
    ...
    ",
        ConnectionStrings.AdventureWorksLiteral,
        TableVarMapping="@input=dbo.MyTableTypeFixed"
     >
...
let ``issue #345 decimal in TVP gets rounded`` () =
    ...
    let tvp =
        [ AdventureWorks.dbo.``User-Defined Table Types``.decimal_test_tvp (value) ]

    use cmd =
        new AdventureWorks.dbo.decimal_test (ConnectionStrings.AdventureWorksLiteral)
...
let ``issue #393 troubleshoot if datetimeoffset raises an exception`` () =
    ...
    let tvp =
        [ AdventureWorks.dbo.``User-Defined Table Types``.datetimeoffset_test_tvp (value) ]

    use cmd =
        new AdventureWorks.dbo.datetimeoffset_test (ConnectionStrings.AdventureWorksLiteral)

    let resultvalue = cmd.Execute(tvp) |> Seq.head
    Assert.Equal(value, resultvalue)

type FixedLengthBinaryTVP =
    SqlCommandProvider<
        "EXEC [dbo].[FixedLengthBinaryTVPTestProc] @fixedLengthBinaryTests",
        ConnectionStrings.AdventureWorksLiteral
     >
...
let ``Using Fixed Length Binary TVP`` () =
    ...
    [ [| 1uy; 2uy; 3uy |]; [| 4uy; 5uy; 6uy |]; [| 7uy; 8uy; 9uy |] ]
    |> Seq.map (fun d -> FixedLengthBinaryTVP.FixedLengthBinaryTVPTest(Some d))
    |> cmd.Execute
    |> ignore


type TestTVPColumnOrder =
    SqlCommandProvider<"EXEC [dbo].[TestTVPColumnOrder] @tvp", ConnectionStrings.AdventureWorksLiteral>
